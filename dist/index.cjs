...
var HabiticaSprite = ({
  ...
  const inlineStyles = import_react.default.useMemo(() => ({
...
var getAvatarSprites = (userSettings, overrideAvatarGear, avatarItemsDetails, imagesMeta, petPrank = null) => {
  const avatarSprites = {};
...
var getAvatarSettingImageUrl = (userSettings, overrideAvatarGear, spriteType, avatarItemsDetails, petPrank = null) => {
  switch (spriteType) {
    case "gear.back":
    case "gear.armor":
    //case 'gear.back_collar':
    ...
    case "gear.weapon":
      ...
      const gearItemDetail = getNestedProperty(avatarItemsDetails, `${spriteType}.${gearItemId}`);
    ...
    case "hair.flower":
      ...
      const hairItemDetail = getNestedProperty(avatarItemsDetails, `${spriteType}.${hairItemId}`);
    ...
    case "mount.body":
      ...
      const mountDetail = mountId ? avatarItemsDetails.mount[mountId] : void 0;
    ...
    case "pet":
      ...
      const petDetail = petId ? avatarItemsDetails.pet[petId] : void 0;
...
var HabiticaAvatar = ({
  ...
  const [staticData, setStaticData] = (0, import_react3.useState)({ imagesMeta: null, avatarItemsDetails: null });
  (0, import_react3.useEffect)(() => {
    const fetchStaticData = async () => {
      const imagesMeta = props.imagesMeta || await (0, import_habitica_avatar_manifest.getHabiticaImagesMeta)();
      const avatarItemsDetails = props.avatarItemsDetails || await (0, import_habitica_avatar_manifest.getHabiticaAvatarItemsDetail)();
      setStaticData({ imagesMeta, avatarItemsDetails });
    };
    fetchStaticData();
  }, [props.imagesMeta, props.avatarItemsDetails]);
